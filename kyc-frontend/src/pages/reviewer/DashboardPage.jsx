...
import { getReviewerDashboard } from '../../api/reviewerApi.js'
...
function pendingDetail(item) {
  if (item.submittedAt) return `Submitted ${formatDateTime(item.submittedAt)}`
  if (item.rejectionReason) {
    return `Returned by ${prettyRole(item.rejectedBy)}: ${item.rejectionReason}`
  }
  return '-'
}
    ...
    ? [
        {
          key: 'pending',
          title: 'Pending',
          description: 'Fresh submissions and items returned by compliance',
          data: dashboard.pending,
          detailPath: (item) => `/reviewer/queue/${item.id}`,
          columns: [
            ...
            { header: 'Detail', render: pendingDetail, wrap: true },
          ],
        },
        {
